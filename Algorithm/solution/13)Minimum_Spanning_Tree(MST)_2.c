...
int opposite(GRAPH* g, int u, int v);
...
int main() {
	...
	for (int i = 0; i < m; i++) {
		printf("%d %d\n",graph->edges[i].endpoint1, graph->edges[i].endpoint2);
	}
	*/
	PrimJarnik(graph, n);
...
int opposite(GRAPH* g, int v, int e) {
	int u = g->edges[e].endpoint1;
	int w = g->edges[e].endpoint2;
	if (v == u)
		return w;
	else
		return u;
